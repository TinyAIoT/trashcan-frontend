...
fill_levels_prediction[0] = {
  timestamp: timestamps[49],
  fill: Math.round(
    Math.min(100, fill_levels_past[48].fill + Math.random() * 2)
  ),
};
for (let i = 1; i < fill_levels_prediction.length; i++) {
  const prev = fill_levels_prediction[i - 1].fill || 0;
  fill_levels_prediction[i] = {
    timestamp: timestamps[i + 49],
    fill: Math.round(Math.min(100, prev + Math.random() * 2)),
  };
}
...
const FillLevelChart = () => {
  ...
  useEffect(() => {
    ...
    svg
      .append("path")
      .datum([
        fill_levels_past[fill_levels_past.length - 1],
        ...fill_levels_prediction,
      ])
      .attr("fill", "none")
      .attr("stroke", "black")
      .attr("stroke-width", 1)
    ...
    const tipPast = d3Tip()
      ...
      .html(
        (_event, d) =>
          `${d.timestamp.toLocaleString()}: <span style='color:red'>${
            d.fill
          }%</span>`
      );
    ...
    svg
      .selectAll(".dot-overlay-prediction")
      .data(fill_levels_prediction)
      .enter()
      .append("circle")
      .attr("class", "dot-overlay")
